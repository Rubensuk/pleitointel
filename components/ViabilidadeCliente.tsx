...
import AssistenteViabilidade from "./ai/AssistenteViabilidade";
...
export default function ViabilidadeCliente() {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="rounded-2xl border border-white/10 bg-slate-900/40 p-6 space-y-6">
          ...
          <form onSubmit={calcularViabilidade} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-mono uppercase text-slate-400 mb-1">
                Estimativa de Votos da Sua Nominata
              </label>
              <input
                type="number"
                value={votosLegenda}
      ...
      {/* Assistente IA Enterprise */}
      {resultado && (
        <AssistenteViabilidade
          dados={{
            votosValidos,
            vagas,
            votosLegenda,
            qe: resultado.quocienteEleitoral,
            qp: resultado.quocientePartidario,
            metaSugerida: resultado.metaSugerida,
          }}
        />
      )}
    </div>
